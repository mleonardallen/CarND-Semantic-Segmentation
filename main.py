...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    """
    conv1x1 = lambda x: tf.layers.conv2d(x, num_classes, 1,
        padding = 'same',
        kernel_initializer = tf.random_normal_initializer(stddev=0.01),
        kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-3))

    upsample = lambda x, scale: tf.layers.conv2d_transpose(x, num_classes, scale * 2,
        strides = (scale, scale), 
        padding = 'same',
        kernel_initializer = tf.random_normal_initializer(stddev=0.01),
        kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-3))

    input = conv1x1(vgg_layer7_out)
    input = upsample(input, 2)
    input = tf.add(input, conv1x1(vgg_layer4_out))
    input = upsample(input, 2)
    input = tf.add(input, conv1x1(vgg_layer3_out))
    input = upsample(input, 8)
